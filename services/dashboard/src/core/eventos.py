...
SELECT scope, proyecto_id,
       ...
       (SELECT max(id) FROM conversation_messages m WHERE scope = 'admin' OR m.proyecto_id = base.proyecto_id) AS conversaciones_ultimo,
       (SELECT count(*) FROM conversation_messages m WHERE scope = 'admin' OR m.proyecto_id = base.proyecto_id) AS conversaciones_total,
