...
def _llamar(ruta: str, payload: dict | None = None, tiempo_limite: float = _TIEMPO_LIMITE) -> str:
...
def reindexar_chunk(proyecto_id: int, chunk_id: int) -> None:
    """Optimización, no requisito: si falla, el RAG se actualiza solo después."""
    fallo = _llamar(f"/internal/proyectos/{proyecto_id}/rag/sync/{chunk_id}")
    if fallo:
        print(f"No se pudo pedir el reindexado inmediato del chunk {chunk_id}: {fallo}")


def olvidar_conversacion(proyecto_id: int, canal: str, client_id: str) -> str:
    ...
    """
    return _llamar(f"/internal/proyectos/{proyecto_id}/conversaciones/{canal}/{client_id}/olvidar")


def responder_como_dueno(proyecto_id: int, canal: str, client_id: str, texto: str) -> str:
    """Envía a través del bot; devuelve vacío si salió o un error presentable."""
    return _llamar(
        f"/internal/proyectos/{proyecto_id}/conversaciones/{canal}/{client_id}/responder",
        {"texto": texto},
        tiempo_limite=25.0,
    )
