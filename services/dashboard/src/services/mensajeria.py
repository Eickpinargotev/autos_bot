...
def listar_plantillas(proyecto_id: int) -> list[dict[str, Any]]:
    plantillas = pool.consultar(
...
def obtener_plantilla(proyecto_id: int, plantilla_id: int) -> dict[str, Any] | None:
    plantilla = pool.consultar_uno(
        "SELECT * FROM plantillas_mensaje WHERE proyecto_id = %s AND id = %s",
        (int(proyecto_id), plantilla_id),
    )
    if plantilla:
        plantilla["partes"] = partes_de(proyecto_id, plantilla_id)
        plantilla["problemas"] = problemas_de(plantilla)
    return plantilla
...
def buscar_por_clave(proyecto_id: int, clave: str) -> dict[str, Any] | None:
    plantilla = pool.consultar_uno(
        "SELECT * FROM plantillas_mensaje WHERE proyecto_id = %s AND clave = %s",
        (int(proyecto_id), (clave or "").strip().upper()),
    )
    return obtener_plantilla(proyecto_id, plantilla["id"]) if plantilla else None


def partes_de(proyecto_id: int, plantilla_id: int) -> list[dict[str, Any]]:
    ...
    partes = pool.consultar(
        "SELECT * FROM plantilla_partes WHERE plantilla_id = %s "
        "AND proyecto_id = %s ORDER BY orden",
        (plantilla_id, int(proyecto_id)),
    )
    for parte in partes:
...
def _clave_valida(proyecto_id: int, clave: str, excepto_id: int | None = None) -> str:
...
def crear_plantilla(proyecto_id: int, clave: str, usuario: str) -> dict[str, Any]:
    return pool.consultar_uno(
        "INSERT INTO plantillas_mensaje (proyecto_id, clave, creado_por) VALUES (%s, %s, %s) RETURNING *",
        (int(proyecto_id), _clave_valida(proyecto_id, clave), usuario),
    )


def renombrar_plantilla(proyecto_id: int, plantilla_id: int, clave: str) -> dict[str, Any] | None:
    ...
    """
    return pool.consultar_uno(
        "UPDATE plantillas_mensaje SET clave = %s, actualizado_en = NOW() "
        "WHERE proyecto_id = %s AND id = %s RETURNING *",
        (_clave_valida(proyecto_id, clave, excepto_id=plantilla_id), int(proyecto_id), plantilla_id),
    )


def eliminar_plantilla(proyecto_id: int, plantilla_id: int) -> int:
    return pool.ejecutar(
        "DELETE FROM plantillas_mensaje WHERE proyecto_id = %s AND id = %s",
        (int(proyecto_id), plantilla_id),
    )


def guardar_parte(
    proyecto_id: int, plantilla_id: int, orden: int, texto: str, media_tipo: str, media_ref: str
) -> dict[str, Any]:
    ...
    """
    if not obtener_plantilla(proyecto_id, plantilla_id):
    ...
    return pool.consultar_uno(
        ...
        INSERT INTO plantilla_partes (
            proyecto_id, plantilla_id, orden, texto, media_tipo, media_ref, media_ok, media_error, media_revisada_en
...
def agregar_parte(proyecto_id: int, plantilla_id: int) -> dict[str, Any]:
    if not obtener_plantilla(proyecto_id, plantilla_id):
        raise ValueError("Ese mensaje ya no existe.")
    fila = pool.consultar_uno(
        "SELECT COALESCE(MAX(orden), 0) + 1 AS siguiente FROM plantilla_partes "
        "WHERE proyecto_id = %s AND plantilla_id = %s",
        (int(proyecto_id), plantilla_id),
    )
    return guardar_parte(proyecto_id, plantilla_id, fila["siguiente"], "", "", "")


def eliminar_parte(proyecto_id: int, parte_id: int) -> int:
    return pool.ejecutar(
        "DELETE FROM plantilla_partes p USING plantillas_mensaje m "
        "WHERE p.id = %s AND p.plantilla_id = m.id AND m.proyecto_id = %s",
        (parte_id, int(proyecto_id)),
    )
...
def revisar_media_de(proyecto_id: int, plantilla_id: int) -> list[dict[str, Any]]:
    """Vuelve a comprobar todos los adjuntos de un mensaje."""
    for parte in partes_de(proyecto_id, plantilla_id):
        guardar_parte(
            proyecto_id, plantilla_id, parte["orden"], parte["texto"], parte["media_tipo"], parte["media_ref"]
        )
    return partes_de(proyecto_id, plantilla_id)
...
def revisar_todos_los_adjuntos(proyecto_id: int) -> tuple[int, int]:
    ...
    """
    partes = pool.consultar(
        "SELECT p.id, p.media_tipo, p.media_ref FROM plantilla_partes p "
        "JOIN plantillas_mensaje m ON m.id = p.plantilla_id "
        "WHERE m.proyecto_id = %s AND COALESCE(p.media_ref, '') <> ''",
        (int(proyecto_id),),
    )
    if not partes:
        return 0, 0
    ...
    with ThreadPoolExecutor(max_workers=REVISION_EN_PARALELO) as ejecutor:
        ...
        for parte_id, ok, error in ejecutor.map(comprobar, partes):
            pool.ejecutar(
                ...
                """,
                (ok, error, int(proyecto_id), parte_id),
            )
            if not ok:
                con_problema += 1
...
def obtener_envio(proyecto_id: int, envio_id: int) -> dict[str, Any] | None:
    return pool.consultar_uno(
        "SELECT * FROM envios WHERE proyecto_id = %s AND id = %s",
        (int(proyecto_id), envio_id),
    )


def reintentar(proyecto_id: int, envio_id: int) -> tuple[bool, str]:
    ...
    fila = pool.consultar_uno(
        ...
        WHERE proyecto_id = %s AND id = %s AND estado = 'error' AND intentos < %s
        RETURNING *
        """,
        (int(proyecto_id), envio_id, MAX_INTENTOS),
    )
    if fila:
        return True, "Reintento encolado."

    actual = obtener_envio(proyecto_id, envio_id)
    if not actual:
...
def reportar(proyecto_id: int, envio_id: int, usuario: str) -> tuple[bool, str]:
    """Escala un envío fallido al administrador y lo deja "en revisión"."""
    envio = obtener_envio(proyecto_id, envio_id)
    if not envio:
        return False, "Ese envío ya no existe."
    ...
    pool.ejecutar(
        """
        INSERT INTO incidencias (proyecto_id, envio_id, reportado_por, detalle)
    ...
    pool.ejecutar(
        "UPDATE envios SET estado = 'en_revision', actualizado_en = NOW() "
        "WHERE proyecto_id = %s AND id = %s",
        (int(proyecto_id), envio_id),
    )
