...
def test_un_chunk_se_guarda_y_se_lee_sin_tema():
    trazabilidad.crear_chunk("El curso teórico cuesta 45.000 colones y dura 12 horas.")

    chunk = trazabilidad.listar_chunks()[0]
...
def test_se_busca_dentro_del_contenido():
    trazabilidad.crear_chunk("El dictamen médico es obligatorio para renovar.")
    trazabilidad.crear_chunk("Las clases prácticas son de dos horas.")

    assert len(trazabilidad.listar_chunks("dictamen")) == 1
    assert len(trazabilidad.listar_chunks("")) == 2
...
def test_no_se_guarda_un_chunk_que_pasa_del_limite():
    ...
    with pytest.raises(ValueError) as fallo:
        trazabilidad.crear_chunk("x" * (trazabilidad.LIMITE_CHUNK + 1))

    assert str(trazabilidad.LIMITE_CHUNK) in str(fallo.value)


def test_justo_en_el_limite_si_se_guarda():
    trazabilidad.crear_chunk("x" * trazabilidad.LIMITE_CHUNK)

    assert trazabilidad.listar_chunks()[0]["largo"] == trazabilidad.LIMITE_CHUNK


def test_tampoco_se_edita_para_pasarse_del_limite():
    chunk = trazabilidad.crear_chunk("corto")

    with pytest.raises(ValueError):
        trazabilidad.actualizar_chunk(chunk["id"], "y" * (trazabilidad.LIMITE_CHUNK + 1))

    assert trazabilidad.listar_chunks()[0]["contenido"] == "corto"
...
def test_sin_chunks_largos_no_aparece_el_aviso(sesion_cliente):
    trazabilidad.crear_chunk("Cuesta 45.000 colones.")
...
def test_el_proyecto_crea_un_chunk_desde_su_panel(sesion_cliente):
    ...
    assert trazabilidad.listar_chunks()[0]["contenido"].startswith("Las clases")
...
def test_pasarse_del_limite_desde_el_panel_avisa_y_no_guarda(sesion_cliente):
    ...
    assert trazabilidad.listar_chunks() == []
...
def test_la_banda_entera_abre_la_edicion(sesion_cliente):
    ...
    chunk = trazabilidad.crear_chunk("Cuesta 45.000 colones.")
...
def test_desactivar_un_chunk_no_lo_borra(sesion_cliente):
    ...
    quedan = trazabilidad.listar_chunks()
    assert len(quedan) == 1
