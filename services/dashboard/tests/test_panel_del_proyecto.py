...
facturacion = ServicioDeProyecto(facturacion, {"actividad_por_cliente"})
trazabilidad = ServicioDeProyecto(trazabilidad, {
    "listar_reportes", "marcar_reporte_revisado",
    "listar_preguntas_sin_respuesta", "marcar_pregunta_atendida",
})
...
def _sembrar_consumo(client_id: str, canal: str = "whatsapp") -> None:
    ...
    pool.ejecutar(
        """
        INSERT INTO uso_eventos (proyecto_id, periodo_id, client_id, canal, categoria, origen,
                                 mensajes, costo_real_microusd, costo_cliente_microusd)
        VALUES (1, %s, %s, %s, 'llm', 'agente', 1, 90000, 500000)
        """,
        (periodo["id"], client_id, canal),
    )
    pool.ejecutar(
        """
        INSERT INTO seguimiento_clientes (proyecto_id, client_id, canal, nombre, ultima_interaccion)
        VALUES (1, %s, %s, 'Quien sea', NOW())
        ON CONFLICT (proyecto_id, client_id, canal) DO UPDATE SET ultima_interaccion = NOW()
        """,
        (client_id, canal),
    )
    pool.ejecutar(
        """INSERT INTO conversation_messages
           ...
           VALUES (1, %s, %s, 'inbound', 'cliente', 'hola')""",
        (client_id, canal),
    )
...
def test_la_pagina_de_conversaciones_no_menciona_el_costo_real(sesion_cliente):
    ...
    for client_id, gasto, hace_horas in (("gasta-mucho", 900000, 48), ("escribio-hoy", 1000, 1)):
        pool.ejecutar(
            """
                INSERT INTO uso_eventos (proyecto_id, periodo_id, client_id, canal, categoria, origen,
                                         mensajes, costo_real_microusd, costo_cliente_microusd)
                VALUES (1, %s, %s, 'whatsapp', 'llm', 'agente', 1, 1, %s)
            """,
        ...
        pool.ejecutar(
            """
                INSERT INTO seguimiento_clientes
                    (proyecto_id, client_id, canal, nombre, ultima_interaccion)
                VALUES (1, %s, 'whatsapp', %s, %s)
            """,
...
def _crear_reporte(problema: str, revisado: bool = False) -> int:
    fila = pool.consultar_uno(
        "INSERT INTO reportes (proyecto_id, nombre, numero, problema) VALUES (1, 'X', '506', %s) RETURNING id",
        (problema,),
    )
    if revisado:
...
def _crear_pregunta(texto: str) -> int:
    return pool.consultar_uno(
        "INSERT INTO preguntas_sin_respuesta (proyecto_id, pregunta) VALUES (1, %s) RETURNING id", (texto,)
    )["id"]
